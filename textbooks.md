## Linear Algebra

- Linear Algebra and Matrix Analysis for Statistics (Banerjee, Roy)
- Linear Algebra and Its Applications (Strang)
- Linear Algebra Done Right (Sheldon Axler)

## Probability and Statistics

- Statistical Inference (Casella, Berger)
- All of Statistics: A Concise Course in Statistical Inference (Wasserman)
- Introduction to Probability by Joseph K. Blitzstein and Jessica Hwang

## Multivaraite Statistics

- Linear Regression Analysis (Seber, Lee)

## Machine Learning

- Pattern Recognition and Machine Learning (PRML) by Christopher Bishop (2006)
- The Elements of Statistical Learning (ESL) by Trevor Hastie, Robert Tibshirani, and Jerome Friedman

## Deep Learning

- Deep Learning by Ian Goodfellow, Yoshua Bengio, and Aaron Courville (2016)
- Understanding Deep Learning by Simon J.D. Prince (2023)
- Probabilistic Machine Learning: An Introduction by Kevin P. Murphy (2022)

## Time Series

- Time Series Analysis and Its Applications by Robert H. Shumway and David S. Stoffer
- Forecasting: Principles and Practice by Rob J. Hyndman and George Athanasopoulos

## Longitudinal Data

- Linear Mixed Models for Longitudinal Data by Geert Verbeke and Geert Molenberghs
- Applied Longitudinal Data Analysis by Judith D. Singer and John B. Willett

## Stochastic Processes

- Adventures in Stochastic Processes by Sidney I. Resnick
- Stochastic Processes by Sheldon M. Ross
- Brownian Motion, Martingales, and Stochastic Calculus by Jean-François Le Gall

## Computational and Bayesian Statistics

- Bayesian Data Analysis (BDA3) by Andrew Gelman, John Carlin, Hal Stern, David Dunson, Aki Vehtari, Donald Rubin
- Monte Carlo Statistical Methods by Christian P. Robert and George Casella
- Computational Statistics by Geof H. Givens and Jennifer A. Hoeting

## Functional Data Analysis

- Theoretical Foundations of Functional Data Analysis, with an Introduction to Linear Operators by Tailen Hsing and Randall Eubank
- Gaussian Process Regression Analysis for Functional Data by Jian Qing Shi and Choi-Yung Choi
- Learning with Kernels: Support Vector Machines, Regularization, Optimization, and Beyond by Bernhard Schölkopf and Alexander J. Smola
- Gaussian Processes for Machine Learning (GPML) by Carl Edward Rasmussen and Christopher K. I. Williams

## Stochastic Calculus 

- Stochastic Calculus for Finance I: The Binomial Model by Steven E. Shreve
- Stochastic Calculus for Finance II: Continuous-Time Models by Steven E. Shreve
- Financial Calculus: An Introduction to Derivative Pricing by Martin Baxter and Andrew Rennie
