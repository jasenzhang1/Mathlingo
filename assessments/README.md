...
The exceptions are machine-learning clusters 10-15 and the whole `python` domain,
...
|---|---|---|---|
| Machine Learning | 100 / 100 | 100 | 100 |
...
| ML-14 | [ml-14-training-at-scale.md](ml-14-training-at-scale.md) | Weight Initialization → Distributed Training (6, all new) | done (48 items) |
| ML-15 | [ml-15-adapting-and-serving.md](ml-15-adapting-and-serving.md) | Scaling Laws → Quantization (7, all new) | done (56 items) |
...
whole map from 236 to 264. Two kinds of gap motivated them. Clusters 13-15 later add **22 more**,
taking the domain to 100: the architecture zoo, what it takes to train one of them, and everything
that happens to a trained model afterwards. Each sets out its own gaps in
[ml-13](ml-13-neural-architectures.md), [ml-14](ml-14-training-at-scale.md) and
[ml-15](ml-15-adapting-and-serving.md).
...
These clusters — 10-15 — invert the usual authorship direction — the concepts postdate clusters 1-9, so
...
rewritten to be self-contained instead, where the edge would have been a stretch. Clusters 13-15
added sixteen more edges on the same terms — four, five and seven respectively — each listed in its
own file alongside the items that were rewritten instead.
